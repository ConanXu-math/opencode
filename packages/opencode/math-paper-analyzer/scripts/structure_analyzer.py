...
Simplified version focused on single paper analysis.
"""
...
def extract_paper_structure(
    ...
    Args:
        ...
        llm_base_url: OpenAI base URL (required if use_llm=True)
        llm_model: Model ID (required if use_llm=True)
    ...
    if use_llm and llm_api_key and llm_base_url and llm_model:
...
def format_summary_for_display(summary: Dict) -> str:
    ...
    if core_techniques:
        lines.append("## 核心方法\n")
        for i, tech in enumerate(core_techniques, 1):
